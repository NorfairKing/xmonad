...
myXmonad = do
  ...
  when setSetKeyboardLayout
    $ spawn
    $ "xmodmap .keyboards/" ++ renderKeyboard setKeyboard
...
keyboardMappingNavigationKeys mod km =
  M.fromList
    ...
    `M.union` (planeKeys mod (Lines 4) Finite)

terminalInSameDir :: String
terminalInSameDir = "urxvt --hold -e zsh --login --interactive -c 'coop ; zsh --interactive --login'"
